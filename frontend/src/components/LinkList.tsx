...
export function LinkList({ onDownload }: Props) {
   ...
   return (
      <section className="w-full bg-white rounded-xl p-6 flex flex-col gap-4 \
         shadow-sm max-h-[calc(100vh-8rem)] overflow-hidden">

         {loading && (
            <span className="absolute top-0 left-0 h-0.5 w-1/3 bg-blue-base \
         ...
         <div className="flex items-center justify-between">
            ...
            <button
               ...
               className="inline-flex items-center gap-2 rounded border \
                  border-gray-300 px-3 py-1.5 text-md text-gray-600 \
                  hover:border-blue-base hover:text-blue-base transition-colors \
...

         {
            links.length === 0 ? (
               ...
               <ul className="flex flex-col divide-y divide-gray-100 \
                  overflow-y-auto min-h-0 flex-1 px-2">
                  {links.map(l => (
