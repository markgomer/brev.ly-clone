...
function ErrorMessage({ message }: { message: string }) {
   return (
      <p role="alert" className="flex items-center gap-1 text-sm text-danger">
         ...
         >
            <path
               d="M6 1L11.196 10H0.804L6 1Z" stroke="currentColor"
               strokeWidth="1.2" fill="none"
            />
            <line
               x1="6" y1="4.5" x2="6" y2="7" stroke="currentColor"
...
export function CreateLinkForm({ onSuccess }: { onSuccess: () => void }) {
   ...
   return (
      <section
         className={[
            "w-full md:w-96 shrink-0 bg-white rounded-xl p-6 flex flex-col",
            "gap-5 shadow-sm"
         ].join(" ")}
      >
         {/* titulo */}
         ...
         <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1">
               ...
               {errors.shortenedLink && (
                  <ErrorMessage message={errors.shortenedLink.message!} />
               )}
            </div>
